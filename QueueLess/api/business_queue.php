...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
     if ($action === 'profile') {
          ...
          if (!$business_id) exit(json_encode(['status' => 'error']));
          ...
          foreach ($queues as &$q) {
              $s = $pdo->prepare("SELECT COUNT(*) FROM queue_entries WHERE queue_id = ? AND status = 'waiting'");
              $s->execute([$q['id']]);
              $q['waiting_count'] = (int)$s->fetchColumn();
    ...
    if ($action === 'create_profile') {
        ...
        echo json_encode(['status' => 'success']);
         ...
         if (!$stmt->fetch()) exit(json_encode(['status' => 'error', 'message' => 'Forbidden']));
